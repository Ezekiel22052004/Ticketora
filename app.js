...
function showSection(sectionId){
  const publicSections=['client-home','client-events','client-verify'];
...
async function processPayment(){if(!activeBuyingEvent||!activeBuyingCategory)return;const name=$('buy-name').value.trim(),email=$('buy-email').value.trim(),phone=$('buy-phone').value.trim(),promo=$('buy-promo').value.trim().toUpperCase();if(!name||!email||!phone)return alert('Veuillez remplir tous les champs.');try{const d=await api('/api/payments/create',{method:'POST',body:JSON.stringify({eventId:activeBuyingEvent.id,name,email,phone,ticketType:activeBuyingCategory.name,quantity:activeBuyQuantity,promo})});if(d.free&&d.tickets){$('modal-buy-ticket')?.classList.remove('hidden');$('buy-step-1')?.classList.add('hidden');$('buy-step-2')?.classList.remove('hidden');renderTickets(d.tickets,true);return;}if(d.payment_url)window.location.href=d.payment_url;else throw new Error('Lien de paiement indisponible.');}catch(e){alert(e.message);}}

function setupPaymentReturn(){const q=new URLSearchParams(location.search);const token=q.get('token');const reference=q.get('reference');const mode=q.get('payment');if((!token&&!reference)||mode!=='return')return;showSection('client-events');$('modal-buy-ticket').classList.remove('hidden');$('buy-step-1').classList.add('hidden');$('buy-step-2').classList.add('hidden');const box=$('buy-step-2');box.classList.remove('hidden');box.innerHTML='<div class="bg-slate-900/50 rounded-2xl p-8 text-center space-y-4"><div class="text-4xl">⏳</div><h3 class="text-xl font-bold text-white">Confirmation du paiement…</h3><p class="text-sm text-slate-400">Ticketora vérifie la confirmation réelle du paiement.</p></div>';let tries=0;let busy=false;const check=async()=>{if(busy)return;busy=true;tries++;try{const endpoint=token?`/api/payments/${encodeURIComponent(token)}/status`:`/api/payments/reference/${encodeURIComponent(reference)}/status`;const d=await api(endpoint);if(d.paid&&(d.tickets?.length||d.ticket)){clearInterval(paymentPollTimer);renderTickets(d.tickets||d.ticket);return;}if(['cancelled','failed'].includes(String(d.status))||tries>=90){clearInterval(paymentPollTimer);box.innerHTML='<div class="bg-rose-500/10 border border-rose-500/20 rounded-2xl p-6 text-center"><p class="text-rose-400 font-bold">Paiement non confirmé.</p><p class="text-sm text-slate-400 mt-2">Aucun billet ne sera délivré tant que Tchin n’aura pas confirmé le paiement.</p><button onclick="closeBuyModal()" class="mt-4 bg-slate-700 text-white px-4 py-2 rounded-xl">Fermer</button></div>';}}catch(e){if(tries>=90){clearInterval(paymentPollTimer);box.innerHTML='<div class="p-6 text-center text-rose-400">Vérification impossible pour le moment. Réessayez dans quelques instants.</div>';}}finally{busy=false;}};check();paymentPollTimer=setInterval(check,4000);}
