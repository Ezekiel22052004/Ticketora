...
let cachedEvents=[];let activeBuyingEvent=null;let activeBuyingCategory=null;let appliedDiscount=0;let appliedPromo=null;let paymentPollTimer=null;
...
async function contributeCagnotte(){if(!activeCagnotte)return;const amount=Number($('cag-amount').value),name=$('cag-name').value.trim(),email=$('cag-email').value.trim();if(!Number.isInteger(amount)||amount<100)return alert('Entrez un montant d’au moins 100 FCFA.');try{const d=await api(`/api/cagnottes/${activeCagnotte.id}/contribute`,{method:'POST',body:JSON.stringify({amount,name,email})});window.location.href=d.payment_url;}catch(e){alert(e.message)}}
function setupCagnotteReturn(){const q=new URLSearchParams(location.search);const token=q.get('token'),mode=q.get('cagnotte');if(!token||mode!=='return')return;showSection('client-home');$('modal-cagnotte')?.classList.remove('hidden');$('cag-modal-title').innerText='Confirmation de contribution';$('cag-modal-desc').innerText='Ticketora vérifie la confirmation réelle de votre paiement…';$('cag-name').value='';$('cag-email').value='';$('cag-amount').value='';let tries=0;cagPollTimer=setInterval(async()=>{tries++;try{const d=await api(`/api/cagnottes/contributions/${encodeURIComponent(token)}/status`);if(d.paid){clearInterval(cagPollTimer);$('cag-modal-desc').innerText='Merci ! Votre contribution a été confirmée.';$('cag-amount').value='';fetchPublicCagnottes();generateContributionReceipt(d.contribution||{},token);return}if(['cancelled','failed'].includes(d.status)||tries>=20){clearInterval(cagPollTimer);$('cag-modal-desc').innerText='Contribution non confirmée. Vous pouvez fermer cette fenêtre et réessayer.';}}catch(e){if(tries>=20){clearInterval(cagPollTimer);$('cag-modal-desc').innerText='Vérification impossible pour le moment.'}}},4000)}
...
window.addEventListener('DOMContentLoaded',()=>{showSection('client-home');fetchPublicEvents();fetchPublicCagnottes();setupPaymentReturn();setupCagnotteReturn();checkOrgSession();});
...
async function loadPartners(){const box=document.getElementById('partners-track');if(!box)return;try{const d=await api('/api/events/partners');const partners=d.partners||[];const fallback=[{name:'Tchin',logo_url:'tchin-logo.jpeg'},{name:'Partenaire',logo_url:''},{name:'Collaborateur',logo_url:''},{name:'Ticketora',logo_url:'logo.png'}];const list=partners.length?partners:fallback;box.innerHTML=[...list,...list].map(p=>p.logo_url?`<div class="partner-logo"><img src="${esc(p.logo_url)}" alt="${esc(p.name)}"></div>`:`<div class="partner-logo partner-text">${esc(p.name)}</div>`).join('');}catch(e){console.warn(e);}}
...
async function generateOrgReport(){const id=Number($('org-report-event')?.value);if(!id)return;try{const d=await api(`/api/organizers/events/${id}/report`),r=d.report||{};$('org-report-box').innerHTML=[['Billets vendus',r.tickets_sold],['Entrées validées',r.validated],['Participants absents',r.absent],['Revenus totaux',fmt(r.revenue)+' FCFA']].map(x=>`<div class="bg-white border rounded-2xl p-5"><p class="text-xs text-slate-400 font-black uppercase">${x[0]}</p><p class="text-2xl font-black text-[#071a3b] mt-2">${x[1]}</p></div>`).join('');}catch(e){alert(e.message)}}

async function generateContributionReceipt(c,token){const old=document.getElementById('contribution-receipt');if(old)old.remove();const wrap=document.createElement('div');wrap.id='contribution-receipt';wrap.className='fixed inset-0 z-[100] bg-black/75 flex items-center justify-center p-4';wrap.innerHTML=`<div class="bg-white rounded-3xl p-5 max-w-md w-full text-center"><canvas id="receipt-canvas" width="900" height="1250" class="w-full rounded-2xl"></canvas><div class="flex gap-2 mt-3"><button id="receipt-download" class="flex-1 bg-[#F97316] text-white font-bold py-3 rounded-xl">Télécharger</button><button id="receipt-share" class="flex-1 bg-slate-900 text-white font-bold py-3 rounded-xl">Partager</button></div><button onclick="this.closest('#contribution-receipt').remove()" class="mt-3 text-sm text-slate-500">Fermer</button></div>`;document.body.appendChild(wrap);const cv=document.getElementById('receipt-canvas'),ctx=cv.getContext('2d');ctx.fillStyle='#fff';ctx.fillRect(0,0,900,1250);ctx.fillStyle='#071a3b';ctx.font='900 54px Arial';ctx.fillText('Ticketora',60,100);ctx.fillStyle='#f97316';ctx.fillRect(60,125,120,8);ctx.fillStyle='#071a3b';ctx.font='900 38px Arial';ctx.fillText('REÇU DE CONTRIBUTION',60,210);ctx.fillStyle='#475569';ctx.font='600 28px Arial';const lines=[['Cagnotte',c.title||'—'],['Montant',fmt(c.amount)+' FCFA'],['Date',c.created_at?new Date(c.created_at).toLocaleString('fr-FR'):'—'],['Référence',c.reference||token],['Statut','PAIEMENT CONFIRMÉ']];let y=300;for(const [k,v] of lines){ctx.fillStyle='#94a3b8';ctx.font='700 22px Arial';ctx.fillText(k.toUpperCase(),60,y);ctx.fillStyle='#0f172a';ctx.font='800 30px Arial';ctx.fillText(String(v).slice(0,45),60,y+38);y+=125;}const qrHost=document.createElement('div');qrHost.style.position='fixed';qrHost.style.left='-9999px';document.body.appendChild(qrHost);new QRCode(qrHost,{text:`${API}/api/cagnottes/contributions/verify/${encodeURIComponent(c.reference||token)}`,width:220,height:220});setTimeout(()=>{const img=qrHost.querySelector('img');if(img){ctx.drawImage(img,600,850,220,220)}ctx.fillStyle='#64748b';ctx.font='600 18px Arial';ctx.fillText('QR Code de vérification',600,1095);qrHost.remove()},300);document.getElementById('receipt-download').onclick=()=>{const a=document.createElement('a');a.download='ticketora-recu-cagnotte.png';a.href=cv.toDataURL('image/png');a.click()};document.getElementById('receipt-share').onclick=async()=>{const blob=await new Promise(r=>cv.toBlob(r,'image/png'));const file=new File([blob],'ticketora-recu.png',{type:'image/png'});if(navigator.share&&navigator.canShare?.({files:[file]}))await navigator.share({title:'Reçu Ticketora',files:[file]});else{const a=document.createElement('a');a.href=`https://wa.me/?text=${encodeURIComponent('Reçu Ticketora '+(c.reference||''))}`;a.target='_blank';a.click()}}}

async function downloadOrgReport(){const id=Number($('org-report-event')?.value);if(!id)return;try{const d=await api(`/api/organizers/events/${id}/report`),r=d.report||{},e=d.event||{};const text=`RAPPORT TICKETORA\nÉvénement: ${e.title||''}\nDate: ${e.date||''}\nLieu: ${e.location||''}\nBillets vendus: ${r.tickets_sold||0}\nEntrées validées: ${r.validated||0}\nParticipants absents: ${r.absent||0}\nRevenus totaux: ${r.revenue||0} FCFA\nGénéré le: ${new Date().toLocaleString('fr-FR')}`;const blob=new Blob([text],{type:'text/plain;charset=utf-8'}),url=URL.createObjectURL(blob),a=document.createElement('a');a.href=url;a.download='ticketora-rapport-'+id+'.txt';a.click();URL.revokeObjectURL(url)}catch(e){alert(e.message)}}
